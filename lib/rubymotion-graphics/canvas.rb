  ...
  class Canvas
    ...
    class << self
      ...
      def for_current_context(options={}, &block)
        options[:type] = :context
        options[:context] = NSGraphicsContext.currentContext.graphicsPort.to_object
        Canvas.new(options, &block)
      end
    ...
    def arc(x, y, radius, start_angle, end_angle)
      start_angle = RMGraphics.radians(90-start_angle)
      end_angle = RMGraphics.radians(90-end_angle)
    ...
    def rotate(deg=0)
      CGContextRotateCTM(@ctx, RMGraphics.radians(-deg));
    end
        ...
        new_state do
          iterations.times do |i|
            if (i > 0)
              # INCREMENT TRANSFORM:
              # translate x, y
              translate(RMGraphics.choose(p.inc[:x]), RMGraphics.choose(p.inc[:y]))
              # choose a rotation factor from the range
              rotate(RMGraphics.choose(p.inc[:rotation]))
              # choose a scaling factor from the range
              sc = RMGraphics.choose(p.inc[:scale])
              sx = RMGraphics.choose(p.inc[:scale_x]) * sc
              sy = p.inc[:scale_y] ? RMGraphics.choose(p.inc[:scale_y]) * sc : sx * sc
              scale(sx, sy)
            end
            ...
            new_state do
              ...
              [:fill,:stroke].each do |kind|
                ...
                if (p.inc[:hue] or p.inc[:saturation] or p.inc[:brightness])
                  # ITERATE COLOR
                  if (p.inc[:hue])
                    newhue = (c.hue + RMGraphics.choose(p.inc[:hue])) % 1
                    c.hue(newhue)
                  end
                  if (p.inc[:saturation])
                    newsat = (c.saturation + RMGraphics.choose(p.inc[:saturation]))
                    c.saturation(newsat)
                  end
                  if (p.inc[:brightness])
                    newbright = (c.brightness + RMGraphics.choose(p.inc[:brightness]))
                    c.brightness(newbright)
                  end
                  if (p.inc[:alpha])
                    newalpha = (c.a + RMGraphics.choose(p.inc[:alpha]))
                    c.a(newalpha)
                  end
                  p.rand[kind] = c
                else
                  # RANDOMIZE COLOR
                  c.hue(RMGraphics.choose(p.rand[:hue])) if p.rand[:hue]
                  c.saturation(RMGraphics.choose(p.rand[:saturation])) if p.rand[:saturation]
                  c.brightness(RMGraphics.choose(p.rand[:brightness])) if p.rand[:brightness]
                end
              ...
              stroke_width(RMGraphics.choose(p.rand[:stroke_width])) if p.rand[:stroke_width]
              # choose an alpha level from the range
              alpha(RMGraphics.choose(p.rand[:alpha])) if p.rand[:alpha]

              # RANDOMIZE TRANSFORM:
              # translate x, y
              translate(RMGraphics.choose(p.rand[:x]), RMGraphics.choose(p.rand[:y]))
              # choose a rotation factor from the range
              rotate(RMGraphics.choose(p.rand[:rotation]))
              # choose a scaling factor from the range
              sc = RMGraphics.choose(p.rand[:scale])
              sx = RMGraphics.choose(p.rand[:scale_x]) * sc
              sy = p.rand[:scale_y] ? RMGraphics.choose(p.rand[:scale_y]) * sc : sx * sc
              scale(sx,sy)
